...
public class HouseDaoImpl implements HouseDao{
	...
	public void loadData() {
		...
		houseInfo = parser.getHouseInfo();//xml파싱을 한다
		deals = parser.getDeals();//4개의 xml이 담긴다
		size = parser.getSize();
	...
	public HouseDeal search(int no) {
		...
		for (String key : deals.keySet()) {
			for (int i = 0; i < deals.get(key).size(); i++) {
				if(deals.get(key).get(i).getNo() == no) {
					hd = deals.get(key).get(i);
					check = true;
				}
			}				
		}
		...
		for (String key : houseInfo.keySet()) {
			if(houseInfo.get(key).getAptName().equals(hd.getAptName())) {
				hd.setBuildYear(houseInfo.get(key).getBuildYear());
				hd.setCode(houseInfo.get(key).getCode());
				hd.setJibun(houseInfo.get(key).getJibun());
				hd.setImg(houseInfo.get(key).getImg());
				return hd;
			}
		}
		if(hd.getImg() == null) {
			hd.setImg("noimg-apply.jpg");
		}
		return hd;
	}
...
// 		System.out.println(dao.search(150));
// 		System.out.println(dao.search(90));
